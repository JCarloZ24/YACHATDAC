# YACHATDAC website — working notes for Codex
...
| Building or reviewing a hi-fi section | `.agents/skills/yachatdac-typography/SKILL.md` **before** writing markup |
...
- `AGENTS.override.md` is the hand-maintained Codex entry point, adapted from
  `CLAUDE.md`. Keep shared project guidance in sync and use `.agents/skills/` here.
- `AGENTS.md` is reserved for Next-generated guidance and is gitignored. Codex loads
  this override instead, so read `AGENTS.md` when present for the framework guidance
  as well. Keep Claude's `@AGENTS.md` import in `CLAUDE.md`; do not add it here.
