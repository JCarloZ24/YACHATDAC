# Resources — "The Record" — draft copy v1

*Last updated: 8 September 2026*

> **Page:** Resources — "The Record" · **Version:** v1 · **State:** draft ·
> **Governance circle:** open
>
> **Source.** Converted from the client's HTML prototype
> `YACHATDAC-The-record.html` (Drive export
> `drive-download-20260824T052412Z-1-001`, received 24 August 2026). Prototype
> title: *The Record — YACHATDAC*.
>
> **What survived the conversion.** The words, the headings, the link
> destinations, the bracketed editorial notes and the image *direction*.
> Layout, styling, scroll behaviour and interaction did **not** — under
> decision **D5** this file governs **copy only**; the wireframes govern
> design. Where the prototype implies a layout, treat it as a suggestion.
>
> **Images.** The prototype's 11 embedded photographs (base64 JPEG, ~1.5 MB)
> were **not** imported. `[ Image — … ]` markers record what each slot is
> meant to carry. Per STATUS note 12 the photography is placeholder direction,
> not an asset manifest.
>
> **Form controls.** `[ Field: … ]` and `[ Button: … ]` mark inputs in the
> prototype, kept because their labels are copy.

---

**The Record**

## What is known about this Country

Stories, historical accounts, research and recordings from Turraburra. Some of it is science, some of it is memory. All of it comes from this Country.

> **Interface amendment, 8 September 2026 — user's handprint opening (F7/F8).**
> Scroll cue: “Scroll into the record”. Bypass link: “Skip to the record”.
> The supplied headline and standfirst above stay verbatim.

### Everything in the record

`[ Field: Search the record ]`

`[ Select: All types / Stories / Historical accounts / Research / Documentation / Recordings ]`

`[ Select: All sources / Iningai knowledge / Colonial record / Published research ]`

`[ Select: Newest first / Oldest first / A–Z ]`

`[ Button: Clear ]`

[ Image ]

Story

Iningai knowledge

#### It nearly didn’t happen

Suzanne Thompson on getting Turraburra back — the phone call on the road, the incognito purchase, and the night they slept on the floor.

The buyback

→ /resources/it-nearly-didnt-happen

[ Image ]

Story

Iningai knowledge

#### Wattanuri, and the sisters he followed

Ten clusters of engravings run south to north along the wall. Read in that order they tell the Seven Sisters.

Seven Sisters · Marra Wonga

→ /resources/wattanuri-and-the-ones-he-followed

[ Image ]

Story

Iningai knowledge

#### Bringing a spring back

A thousand litres carted twice a day through drought, until it held on its own. Eight days, then a koala.

Water & springs · Rangers

→ /resources/bringing-a-spring-back

[ Image ]

Story

Published research

#### You are standing on a seabed

The Eromanga Sea covered inland Queensland. The escarpment is its floor, and the footprints set in the mud are still here.

Deep time & fossils

→ /resources/you-are-standing-on-a-seabed

[ Image ]

Story

Iningai knowledge

#### A day with the Rangers

Fire workshops, fencing, seed, machinery. Two days are never the same.

Rangers · Fire

→ /resources/a-day-with-the-rangers

Historical account

Colonial record

#### What Mitchell wrote down

The explorer passed through in the 1840s and described huts built solidly enough to carry bark tiles. The first written account of the people here was written by a stranger.

First contact & the frontier

→ /resources/what-mitchell-wrote-down

[ Image ]

Historical account

Colonial record

#### When they called it The Art Gallery

By the 1950s the escarpment had a name given to it by visitors. It was written up and admired under that name for decades.

Marra Wonga · First contact & the frontier

→ /resources/when-they-called-it-the-art-gallery

[ Image ]

Historical account

Colonial record

#### Gracevale becomes Turraburra

Bought back in April 2019, renamed on 1 October 2020 after the Terraburra clan recorded on this Country in an 1884 map.

The buyback

→ /resources/gracevale-becomes-turraburra

[ Image ]

Research

Published research

#### Fifteen thousand markings, read in order

Our account of the two-year study of Marra Wonga, documented with Iningai people as research partners.

Marra Wonga · Seven Sisters

→ /resources/fifteen-thousand-markings-read-in-order

[ Image ]

Research

Iningai knowledge

#### Right-way fire and the carbon in the soil

Two flux towers, one on Country managed our way and one on grazed land, read against each other for more than a year.

Fire · Carbon & climate

→ /resources/right-way-fire-and-the-carbon-in-the-soil

[ Image ]

Documentation

Iningai knowledge

#### A season of bush foods

What comes on when, across the year, and the Country each one grows out of.

Bush foods

→ /resources/a-season-of-bush-foods

Documentation

Published research

#### Pollen at sixty metres

A 480-metre bore kept a soil sample every six metres. Sixty metres down, there was pollen. Samples held with QUT.

Deep time & fossils

→ /resources/pollen-at-sixty-metres

[ Image ]

Recording

Iningai knowledge

#### What the recorders hear at night

Four acoustic recorders listen across the property, two of them at the springs. Frogs, birds at first light, and a good deal not yet identified.

Animals & birds

→ /resources/what-the-recorders-hear-at-night

[ Image — Nothing here yet under that. Try another subject, or ask us what exists. ]

### What we do not know

Most of this Country has never been studied. These are the gaps we would like filled, and the reason we keep the record in the first place.

**How old is the wall?** — Marra Wonga has never been scientifically dated. Mud wasp nests over some engravings could give minimum ages if anyone samples them.

**What lives here?** — Four recorders are logging species and a good deal of it is unidentified. There is no full list of what is on this property.

**What is in the ground?** — Fossil footprints, petrified trees, and what one visiting specialist thought might be dinosaur eggs. Almost none of it examined.

**What does fire-stick farming actually do?** — Two flux towers are measuring it. More work is welcome, particularly on soil carbon at depth.

[Research with us](/partnerships/#research-opportunities)

### Documents and reports

11 items

##### Ten Year Strategic Plan

Where the work is going, in two phases. Published in full.

2026–2036 · PDF · Download

##### Governance

Board composition, the Rule Book, Elder Advisory Group terms of reference, and our registration details.

Board · Rule Book · ORIC & ACNC · Download

##### Research bibliography

Every paper written about Turraburra, cited and linked to the publisher.

Updated continuously · Download

##### Financial statements

Audited accounts, published each year.

Annual · PDF · Download

##### Annual report

What was done, what it cost, what changed. Published every year from here.

First edition in preparation · In preparation

##### Country condition scorecard

Condition and trend for each thing we are managing for, and each threat against it.

Annual · In preparation

##### Knowledge needs

What we do not know and would like studied. Start here if you are a researcher.

Updated as work progresses · In preparation

##### Working with us — research protocol

How work on Turraburra is agreed: consent, access, who holds data, and what the Elder Advisory Group signs off.

For researchers and partners · In preparation

##### Land Management Plan

The plan the Ranger workplans sit under.

Year 2 · In preparation

##### Workforce plan

Iningai employment targets and the pathway to them.

Year 1 · In preparation

##### Five-Year Review

External review, community consultation, and a revised plan. Committed to now.

2031 · 2031

### Items marked “on request”

Some material is not published openly — usually because it is raw data held with a research partner, or because it carries cultural detail that is shared with context rather than downloaded.

Requests go to YACHATDAC and, where the material is cultural, to the Elder Advisory Group. Tell us who you are and what you need it for. **[ Response time and contact point to confirm. ]**

**Keeping the record**

### The record grows

New material goes up as the work happens — a season of burning, a paper published, a recording nobody has identified yet.

#### Get new material as it lands

A few times a year, when something is added. What went up, and why it matters. No campaigns.

`[ Field: Email address ]`

`[ Button: Subscribe ]`

We will not pass your address to anyone.

#### Do you hold something?

Plenty of what belongs to this Country sits in other people's sheds, albums and filing cabinets.

- Photographs of the escarpment or the station, any era
- Station records, letters, diaries or maps
- Family papers mentioning Iningai people
- Old tour brochures, newspaper clippings, survey reports

[Tell us what you have →](/connect)
