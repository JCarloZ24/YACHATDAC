...
| `drafts/about/YACHATDAC-About-Copy-v1.md` | About | 📥 **v1** uploaded; editorial notes taken off the page 11 Sep 2026 | open | ICN/ABN missing; legal-name spelling unconfirmed — notes 10, 13. ⚠ **/about no longer carries its own not-for-publication markers** (user direction, 11 Sep 2026): all four came off `src/app/about/_components/Sections.tsx`. The four `pending` strings are kept in `src/content/about.ts` with a comment on each, and THIS ROW plus R15 are now the record — nothing on the built page says the copy is unconfirmed, so a pre-launch grep of `data-placeholder` will not find them |
