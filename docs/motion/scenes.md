...
Closing-pattern verification, 13 September 2026: desktop pointer positions produce opposite spiral transforms while the heading's bounds and ground entrance transform remain identical. Pointer leave and blur return both to identity; hidden beats ignore input and reverse scroll returns neutral. Reduced motion removes both transforms and restoring preference rebuilds them. A 375×812 / DPR 2 touch visit stays still before and after a tap, with no horizontal overflow. Client navigation through Connect and back rebuilds the response with one home pin. Typecheck, lint, typography check and production build pass. Local review only; no push requested.

Home sunrise/Milky Way overlap, 13 September 2026 (user screenshots and simultaneous sun/galaxy reference): the welcome's warmer sky was already above the diffuse contrast threshold, leaving fine stars but removing the galactic band. The band and its concentrated fine stars now retain detail through the low-sun period, warm toward amber, and release continuously as solar elevation increases from 4° to 16°. Local bright glare still conceals detail; deep-night colour, star/dust seeds, the sun path, scene ownership and woodland occlusion stay unchanged. This is the requested photographic-style composite, not literal naked-eye astronomical visibility. No new assets, renderer, loop, reading span or changed copy.

Sunrise overlap verification, 13 September 2026: checked the settled welcome, sunrise, sunset, full daylight, deep night and reverse scroll at desktop size, plus fresh and return touch visits at 375×812 / DPR 2. The glare allowance preserves galactic detail above the bright mobile horizon while full daylight still clears it. Reduced-motion fallback and restoration pass; no final shader/runtime errors. Typecheck, lint, typography check and production build pass. Changes remain local for review.

Home galactic realism, 13 September 2026 (three additional user night-sky references): replace the smooth ribbon-like glow with finer, clumped stellar light and irregular absorbing rifts. Rebuild the stars as three independently seeded populations with many more small, antialiased points, varied flux and restrained spectral colour. Faint stars cluster around the galactic band; sparse foreground stars can appear in front of its dust. Local sky contrast still determines appearance, the pattern stays fixed, and the solar clock/ownership, woodland masking, meteors, copy and reduced-motion cut do not change. Procedural scenic interpretation, not a copied reference photograph or astronomical record.

Galactic realism verification, 13 September 2026: desktop captures cover daylight, first dusk stars, deeper dusk, late Truth, Belonging and reverse sunrise; a fresh 375×812 / DPR 2 touch visit retains sharp star detail, woodland occlusion and no horizontal overflow. Reduced motion removes the pin; restoring preference rebuilds it. No shader/runtime errors in the final browser check. A 90-frame desktop sample at 4× CPU throttling averaged 11.0ms between animation frames (not a physical-phone GPU benchmark). Typecheck, lint, typography and production build pass. Changes remain local for review.

Home Milky Way sky, 13 September 2026 (user photograph reference, **the stars emerge as daylight leaves**, galactic depth): extend the existing procedural night with an inclined stellar band, a warm core, irregular absorbing dust lanes and two fine point-star scales. The reference is not used as a shipped asset. Diffuse light responds to a darker local-sky threshold than prominent stars; all structure is fixed across nights and reverse scrub, with the same horizon attenuation and woodland occlusion. No cloud experiment is restored, and no solar timing, ownership, reading hold, copy, renderer, texture request or ambient loop changes. Generated scenic interpretation only, not cultural astronomy or a reconstruction of Country's sky. Reduced-motion and failed-WebGL fallback remain static.

Milky Way verification, 13 September 2026: captured daylight, sunset, twilight, late Truth, Belonging and reverse scroll at 1440×890, plus fresh and return touch contexts at 375×812 / DPR 2. The diffuse threshold sits above the supplied night haze floor so the band does not dim again at the dusk/night handoff. No shader errors or mobile overflow; reduced motion removes the pin and restoring preference rebuilds the scene. Typecheck, lint, typography check and the production build pass. Local review only; not committed or pushed.

Solar handoff verification, 13 September 2026: before the fix, an early scroll to timeline time 2.05 reached a sun elevation near 8° and then fell back to welcome twilight while the scroll playhead stayed at 2.05. After the fix the same rendered sun uniforms remain stable beyond the opening's end. Checked normal top-of-page entry, early scroll on desktop, first and return touch visits at 375×812 / DPR 2, a position restored before entry, forward/reverse day and night stops, refresh and reduced-motion fallback/restoration. New pins can have an unmeasured end and a tiny positive progress at scroll zero; the handoff requires finite travel beyond 1px so neither condition skips the opening. Typecheck, lint, typography check and production build pass. No push requested.

Home solar handoff fix, 13 September 2026 (user screenshot, **Country carries the day** / SCR-09): reproduced by scrolling to Wonder while the opening is still running. Both timelines wrote to the same scene object; after the scroll reached sunrise, the opening wrote welcome twilight back over it. The renderer now selects between independent opening and scroll states, with a one-way handoff when the opening finishes or scrolling begins. Early/restored scroll settles the opening presentation and the entry callback is idempotent. Normal arrival retains the night-to-welcome opening; reverse scroll returns to welcome without replaying it. No sky appearance or reading-time change, and no clouds.

Home spatial star visibility, 13 September 2026 (user screenshot follow-up, **the stars emerge as daylight leaves**): replaces the solar-depression star fade described below with local sky-luminance contrast. Darker patches of the existing sunset reveal stars while brighter patches still conceal them; the revealed area grows as the actual sky darkens. Bright and faint stars use different contrast limits, keeping variation without a common opacity fade. Only the exposed sky is sampled, not terrain or text, and the woodland alpha still occludes all stellar light. The star pattern, sun path, reading clock, ambient pause/cleanup and static fallback are unchanged. No push authorised for this revision.

Home stars throughout night, 13 September 2026 (user direction, **the stars emerge as daylight leaves** / SCR-09 / SCR-10 / AMB-05): opening night, late Truth and Belonging now share one procedural sky. Solar depression controls each star's visibility by apparent brightness, with bright stars emerging in late civil twilight and faint ones waiting for deeper darkness. Dawn reverses the order without reseeding or a section-triggered fade. The opening stop starts below astronomical twilight; all later solar stops and reading holds are unchanged. Low stars are attenuated by horizon haze and twinkle slightly more than high stars, using the existing pausable ambient phase. Stars remain behind the woodland alpha and leave with the landscape; the later bare charcoal panels do not become skies. Belonging's existing meteors remain gated to Belonging. Retire the image-sky request so there is no second baked star field to crossfade. No new assets, renderer, clock or dependency. Generated scenic stars are not a site-specific astronomical or cultural record. Reduced-motion and failed-WebGL cuts retain the readable static document.

Home solar timelapse, 13 September 2026 (user direction, **Country carries the day** / SCR-09 / SCR-10): captured the welcome, Wonder, Truth introduction, all seven years, Belonging, Invitation, statement, offer and pathways before changing the scene. Welcome stays in dawn twilight; Wonder carries sunrise; early Truth reaches daylight; the later years lower the sun into sunset and blue hour; Belonging and the departing land remain night. A procedural atmosphere and solar disc are composited behind the supplied woodland alpha in the existing Three.js material. The solar clock derives from the existing sky offsets, so the page's duration, reading holds and reverse scroll remain shared. Atmospheric colour and approximate exposure replace the stylised daytime sky/light treatment; the original night layer and static fallback survive. A soft upper media scrim protects cream Truth copy, especially where the low sun meets the phone's reading band, and drains away into night. The sun is generated scenic lighting, not an astronomical reconstruction. The photograph has no 3D terrain, so this pass does not manufacture moving cast shadows. Clouds are deferred. No new image requests, renderers or animation loops.

Home touch startup crash, 12 September 2026 (user follow-up, SCR-09): a fresh touch-only browser threw `timeline.frameOpen is not a function` while building the dissolve. Home registered only its own effect family; the media effect it calls was incidentally registered by the desktop-only ClickBloom component, so desktop viewport resizing missed the failure. Home now registers the shared vocabulary directly. The GSAP context is assigned before effect construction so a partial failure can revert the welcome text as well as the canvas. The earlier mobile fallback-layout fix treated the visible symptom; this corrects the startup failure that was keeping touch visits in that fallback. Verification must use fresh touch-enabled contexts, including first visits and return visits, not just a narrow desktop viewport.

Home mobile readiness and layout, 12 September 2026 (user screenshot, SCR-09): the no-canvas layout now reads in document order, with a separate opening viewport, the existing static beats, then the closing panels. Loading, failed WebGL and reduced motion no longer stack absolute panels over the welcome. After the texture maps decode, the opening wrapper becomes `display: contents` and the existing pinned scene takes over; sizing happens against that viewport before the first render. Releasing the scene restores the flowing page. This resolves the genuine-failure fallback problem recorded in the 10 September tab-switch note below. On mobile, Pathways reserves the fixed 80px header band, caps itself to the small viewport and shares card rows so the images use the height left by the full text and links. Short phones use the existing H3 size for the long heading and tighter gaps. The SCR-09 timing and horizontal travel are unchanged.
...
| 19 | The seabed **builds downward**, top → middle → bottom: the one section that accumulates in the same direction as the scroll. From 18 September 2026 each stratum arrives on the montage cut (a clip from its own edge), in that order, not the brightness arrival. |
...
| 04 | Getting here — the route map | 420 | charcoal, sticky span | **media** | the same draw and flare; the copy arrives in stop order. Equalised with §02 on 17 September 2026 (user direction, "extend the animation on getting here map both mobile/desktop, equally to first map section"): the same 420 span drawn over 320 and held finished for 100; the road inks over the same 0.06–0.8 window and the four stops light at 0.70–0.86, the facts map's own feature marks, so the phone cut — which measures each map on its own box — is paced alike too. Was 180 (10 Sep 2026), before that 300 |
...
| 07 | Before you come | 100 + 80 still (30 on the phone, 17 Sep 2026) + 100 hold | evergreen, held at its foot | **none** | `hold`; cells arrive and their figures `tally` up, paragraphs arrive after; the call to action `pop`s once on its own clock; then the section holds at its foot while §08 rides over it, cream crest first (16 Sep 2026) |
| 08 | Where you stay | 120 + 60 still on the phone only (17 Sep 2026) + 100 hold | canvas, cream crest overhanging (was the frame's evergreen drip, 16 Sep 2026), held at its foot | **media** | heading and copy enter; the ten-frame carousel does not — no card entrance (10 Sep 2026); holds at its foot while §09's photograph rides over it (16 Sep 2026) |
...
Home Truth second line - 15 September 2026 (user direction): the chronology runs on **two parallel horizontal dotted lines**, not one. Figma frame 05, `2048:11158`, draws the rail as a pair, and the line this page already had is that frame's Rail B (`2048:11629`) — 112 dots, and the strand the rosette rides, which is what the year marker already does here. The addition is Rail A (`2048:12273`), sitting 4.306% of the hero width below it, in `--color-burnt` for the frame's Colour/Burnt Ochre `#CB7722`. It is the SAME supplied artwork recoloured through a mask, not a new asset: read live from Figma, the two rails are one strand used twice, with identical dot x-coordinates and only a 224.32 shift along the strand's length between them. **Nothing about the motion changes.** The new line sits inside `[data-truth-timeline]` and arrives on the fade that wrapper already plays, so it owns no tween; the year marker still travels its seven anchors; the path still has no horizontal travel of its own. Frame 05's lengthwise stagger is not ported — it gives a *finite* vertical rail two fade-out points, and this line runs off both edges, so staggering it would only pull its baked end-fade into view. This supersedes nothing except the word "path" in the singular.
