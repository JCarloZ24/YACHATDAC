...
Cloud experiment shelved, 13 September 2026 (latest user direction): preserve the implementation on `experiment/home-clouds` for possible future consideration. It is not approved for `main`; return the active site to the preceding sun-and-stars treatment without clouds. This supersedes the earlier no-push note for this experiment only.

Cloud verification, 13 September 2026: inspected the complete desktop scene sequence and fresh 375×812 / DPR 2 mobile crops. Removed screen-space ray jitter after it showed a grainy grid on mobile; the final pass uses 56 fixed samples, three short light probes, distance-filtered detail and a 900px maximum target edge. Its generated RG noise occupies 512 KiB before mipmaps. Checked reverse scroll, continuous travel through the 24-second ambient boundary, GPU resource disposal, no cloud rendering after the landscape leaves, reduced-motion fallback/restoration and the clear-sky fallback without float colour buffers. Cloud-only GPU timing on the development machine was approximately 3ms at 1440×901; this is not a phone-hardware performance guarantee. The existing local-darkness star rule remains in the compositor.

Home cloud timelapse, 13 September 2026 (user direction, **clouds carry the passing hours** / SCR-09 / SCR-10 / AMB-05): a broken volumetric cloud layer joins the existing sun and stars. A small, generated, seamless 3D noise texture supplies billows and eroded edges; a capped half-resolution pass in the same renderer integrates density and sunlight through the cloud deck. Perspective makes nearby forms travel faster than the distant horizon. The existing solar offset drives timelapse travel and the existing breeze's elapsed time supplies gentle drift while reading, without resetting at its 24-second boundary. Cloud illumination follows the sun through dawn, day, dusk and night. Clouds occlude both the solar disc and stellar light, while unobstructed stars still depend on their local sky's darkness. No new loop, image request, scroll span or land deformation. Generated scenic weather is not a record of Country. Hidden/offscreen pause, reduced-motion stills, failed-WebGL readability and scoped GPU disposal remain. This supersedes the deferred-cloud note below; no push requested.
