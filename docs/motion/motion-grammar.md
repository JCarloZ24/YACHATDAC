...
| --- | --- | --- | --- | --- |
| **Country carries the day** | One procedural sun rises behind the woodland at Wonder, crosses above the frame during early Truth, and sets behind the opposite treeline during the later years. Atmospheric scattering changes the sun from warm near the horizon to white in daylight; haze and photographic exposure follow the same light. Belonging retains night. The supplied land alpha occludes the sun, including branches. | Derived from the existing sky-offset clock in `homeHeroOpen` and `homeHeroDissolve`; 80vh/unit, reversible scrub 0.8s, no added holds or ambient solar loop. Static fallback for reduced motion/WebGL failure. | User requests realistic canvas/Three.js sunrise and sundown, 13 September 2026. Supersedes exact Figma sky/light colours on Home for this treatment. | `homeHeroOpen`, `homeHeroDissolve` → `home-sun.ts` / `home-land.ts`, existing controller-owned renderer |

The sun and atmosphere are generated scene lighting, not an astronomical record or a claim about the photograph's original lighting. The land remains the supplied photograph; no terrain geometry or changing cast shadows are inferred. Clouds are deferred to a separate pass on user direction. Atmospheric colour variation is part of this requested scene, an exception to the ban on decorative gradient washes. Follow-up, 13 September 2026: enlarge the solar disc to 2.5 times its initial diameter for visibility; its path, scattering and terrain occlusion remain the same.

Solar appearance follow-up, 13 September 2026: the enlarged disc now changes continuously with elevation. Dawn/dusk take a warm, softly edged disc with slight vertical refraction and a restrained circumsolar haze; daylight resolves to a rounder, whiter disc. Atmospheric extinction is sampled across the disc so its lower edge reddens first, and limb darkening keeps it from reading as a flat graphic. Display scale ranges from 2× in daylight to the requested 2.5× near the horizon (a scenic choice, not literal astronomical magnification). All appearance parameters share the reversible solar clock.

| Home night amendment — 13 September 2026 | Behaviour | Timing | Authority | Implementation |
| --- | --- | --- | --- | --- |
| **the stars emerge as daylight leaves** | The same seeded star field inhabits opening night, late Truth and Belonging, continuing with the departing landscape. Stars become detectable against their own patch of sky: darker areas reveal them first while the brighter horizon/solar glow still conceals them. Bright stars tolerate more sky light than faint ones. Dawn reverses the spatial reveal. Horizon extinction softens low stars; woodland alpha occludes them. No section-boundary star replacement. | Per-fragment, magnitude-dependent contrast against the exposed sky's linear luminance, before terrain and text. No solar-depression fade or timed star-opacity tween. The existing 2.4s opening and 80vh/unit reversible scroll clock change the atmosphere itself; AMB-05 supplies restrained, independent twinkle. No added loop or reading hold; reduced motion keeps the static document. | User requests stars in every night, then local darkness rather than a whole-sky fade (screenshot), 13 September 2026 | `homeHeroOpen`, `homeHeroDissolve`, `homeLandscapeBreeze` → `home-sun.ts`, `home-stars.ts`, `home-land.ts` |

This supersedes the Belonging-only star visibility below, not its shooting-star timing. The opening solar stop now starts below astronomical twilight; the welcome/sunrise/day/sunset stops are unchanged. Generated star positions and display brightness are scenic, not a star catalogue, constellation depiction or cultural record. Transition reference: [NWS twilight definitions](https://www.weather.gov/fsd/twilight); atmospheric twinkle reference: [NASA StarChild](https://starchild.gsfc.nasa.gov/docs/StarChild/questions/question26.html). The image sky is no longer fetched by Home, preventing baked stars and generated stars from crossfading through different positions. Night land lighting still uses the supplied light sequence.

Spatial visibility correction, 13 September 2026: the initial 4–18° solar-depression thresholds made stars emerge together across equally elevated parts of the sky, even when the sunset left one side much brighter. The requested replacement compares each star with the actual local, exposed sky colour. The sky-only comparison excludes the photograph, lettering and UI, so dark trees or a black panel cannot create stars. Brightness thresholds are a scenic display calibration, not photometric measurements of Country.
...
| **emerging from the ground**, the phase swap | Two records sharing one pinned screen are read ONE AT A TIME rather than side by side: each brightens its own eyebrow, heading and body in reading order, holds, then dims back out, and the next arrives on ground the first has left. Opacity only, in both directions, and on the ELEMENTS rather than the article — the article's `transform` is spoken for by its hover. ⚠ **SEQUENCED, NEVER CROSSFADED.** About §03 measured both of its claims at full opacity for .14 of its read when the exit and the entrance were run together, because a staggered beat holds its last member opaque until the end; two records on one grid cell then print on top of each other. There is deliberate empty screen at the swap and the outgoing container is cleared explicitly, rather than trusting the incoming one to cover it. ⚠ **NO SPLIT BEAT EITHER WAY**, because `freshSplit` keeps one live SplitText per element and reverts the previous one: a block that both arrives and leaves would silently lose its arrival while its exit worked perfectly. | `machine`, scrubbed · dim 0.4 · elements .015 apart; .06 of empty screen at the swap | M1 · Truth §02–§03 · user direction 13 September 2026 | diptych | `brighten` |
...
| ~~**what has not happened yet**~~ | ⟡ **RETIRED 13 September 2026, user direction, and the row is kept rather than deleted so the next person to reach for IMG-04 finds the history.** The chromatic split was held on About §06's Elder Advisory Group sentence for two days as a tense marker: two ghost copies, oxide and turquoise, a couple of pixels off the sentence and drifting on a cycle with no end state, because the group is not sitting. It came off for two reasons, both reported from a 1366 x 643 screen: the two ghost layers did not align at small sizes, and the duotone is used nowhere else on the site, so it read as a rendering fault rather than as a register. ⚠ **THE TENSE DID NOT COME OFF WITH IT.** The sentence still reads "is being established" and "once it is sitting", which is the claim the marker was decorating rather than making, and D5 puts the draft in charge of those words. kit.ts keeps its consent record for `about-people-02` unchanged. Removing a visual device is not removing what it pointed at, and the removal condition the row used to carry — when the group sits, the marker comes off — has simply been met early by a different route. **IMG-04 is now unspent on this site.** It stays released by F9 and available on depicted subjects; it is not available on a person's recorded words, on testimony, or on a figure of loss. | ~~2px offset, ~1px drift, no end state~~ | About §06 · built 12 September 2026, retired 13 September 2026, both on user direction | — | ~~CSS `ab6-drift`~~ |
...
| **the world opening**, one face at a time | About §07's roster. A row of portraits stands on one held screen and the reader's scroll walks a FOCUS along it: the frame the focus is on grows to full size and carries a legible name, the ones either side wait at a fifth of that and unnamed, and every state between is continuous — there is no snap, so the rail retraces exactly on the way back, which is the contract the five held screens before it keep. The argument is the page's, not the effect's: this section says a named woman founded the organisation and other named people do the work, and a rail that can only show one face whole at a time is that sentence as a mechanism. It is also why the roster is a LIST rather than a composition — a face is added by naming a person who is already on /our-people and pointing at a cleared photograph, and the rail re-divides its own window. ⚠ **THE PLATE GROWS FROM ITS OWN BOTTOM EDGE AND KEEPS ITS ASPECT.** These are `frame`-grade portraits of identifiable people, so nothing is written to the image plane and no frame is ever re-proportioned: a plate that changed aspect would re-crop the picture as it grew, which on a portrait means the crop travelling across somebody's face. It scales uniformly about `bottom center`, so the picture only gets larger and the name below it never moves. ⚠ **A NAME AND A STAND-IN MARKER ARE ONE OBJECT HERE.** Where the photograph under a name is a stand-in, the marker rides the plate and counter-scales so it stays the same size at every focus — it is not allowed to shrink into illegibility at the exact moment the face is small. kit.ts: "a screenshot circulated without it is exactly how a placeholder becomes a claim." Mechanically it is SCR-15's scrubbed horizontal travel (`recordDocumentsTraverse`) crossed with §04's one-card-whole-at-a-time, and it borrows §04's composed-transform fix: the frame's travel and the plate's scale are separate elements driven by separate CSS custom properties, because three `quickSetter`s on one element's transform do not compose. All geometry is CSS in `svh` and the module writes two unitless numbers per frame, so nothing is measured per frame. | smoothstep falloff over one pitch either side · rest 20svh tall at 3/4, focused ×2.3, pitch 26svh · scrubbed across .30 – .86 of the read, divided by the roster | About §07 · SCR-15 · user direction 12 September 2026 | P4 | `theRoster` |
| **the world opening**, one frame at a time, Truth cut | The Ahead deck's precinct photographs. The same walked focus as About §07's roster, on a row of two: the tile the focus is on grows about its own bottom edge and the other waits smaller, every state between continuous, so the row retraces exactly on the reverse scrub. ⚠ **THE SCALE IS ON THE TILE, NEVER ON THE IMAGE PLANE, and here that is a hover rule rather than a motion grade.** These are `full`-graded `work` and `country` frames, so the plane could take it — but `[data-truth-card] [data-media-plane]` already carries a `transform: scale(1.03)` hover and `[data-truth-card]` a `translateY(-4px)` one. Three elements, three transforms, none written by the same author: the article hovers, the plane hovers, the tile walks. A GSAP `scale` on either of the first two replaces the stylesheet's transform and that hover never appears again — `globals.css`'s own standing warning, which this section was already tripping. ⚠ **AND `sizes` DESCRIBES THE FOCUSED TILE, NOT THE RESTING ONE.** A `transform: scale()` is invisible to the browser's image selection: it sizes the request from the layout box and CSS then paints that image larger. About §07 shipped this and it was reported as "why is the second image blurry". | smoothstep falloff over one pitch either side · scrubbed across .06 – .42 of the read, divided by the row | Truth §02 · the roster row above · user direction 13 September 2026 | diptych | Truth-local recipe, `theRoster` lineage |
...
| **accumulating**, the register fills a column at a time | About §08's partners. Three groups — Research, Government and land, Industry and community — arrive TOGETHER, rule and title, and then the names fill in by ROUND: the first partner of every group, then the second of every group, and so on, so the three lists advance in step instead of one completing before the next begins. The reading is the section's own claim: these are three kinds of partner held at the same time, not a ranked list, and a register that filled one column to the bottom before starting the next would say the opposite. The groups are 4, 3 and 2 long, so there are four rounds and the shorter groups simply stop — nothing is padded to square the table, because the point is who is actually there. ⚠ **A SEPARATOR BELONGS TO THE ROUND THAT BRINGS THE NAME AFTER IT.** The middot sits inside the item it follows (a leading one orphans at a line start, and each name has to wrap as a unit), so revealing a name with its own trailing middot shows "QUT ·" pointing at nothing. It is held back one round. At no fraction of this scrub is a visible separator the last visible thing in its list. ⚠ **HELD, AND ON ITS OWN FLOOR.** Two cuts were wrong before this one. The first anchored the scrub on the groups block's own passage, which does not work on a deck: `coverSeams` pins a section the moment its foot meets the viewport's, so nothing inside it travels further than `height − one viewport` however the trigger is written, and round four never arrived. The second gave the section 200vh and let the column travel through it, which the user then watched scroll away mid-sequence ("make it still/pinned until everything is done"). It is a held screen like §02 – §07, and the room came out of the SPACING rather than the type — every gap halves, the column goes from 984px to 779 at 1280 and 662 at 1440, and the frame's 30px names and 3.5rem heading are untouched. ⚠ **Its floor is 1280 wide, not 1024 like the other six**, because nothing in it is sized in `svh`: measured, the column is 803px at 1440, 920 at 1280 and 1056 at 1024, where the heading goes to three lines and every list wraps. At 1024 it does not stand on any laptop screen, so below 1280 the section is the complete flow document. The rules keep the clip-draw §08 already had — `inset(0% 100% 0% 0%)` → `inset(0% 0% 0% 0%)` — moved off their own viewport triggers onto this clock, and all three draw together rather than staggered, because their arriving at once is what "the groups appear at once" means. ⚠ **The TITLES take that same clip, not a fade**, and share the tween with the rules: a rule wiping left to right beside a title fading in place is two gestures at the same time, which is not the same thing as one gesture ("both lines and text should be synced as it reveal", user, 12 September 2026). One edge travels across the line and its name together. | `country`, scrubbed across the section's read (300vh held, 200 read) · rules AND titles share one clip tween over .18 – .34 · rounds at .38 .50 .62 .74, each .09 long · names `arrive` (X4), no line mask — a single-line name has nothing to mask. ⚠ The first cut drew the rules over .12 and faded the titles over .04 on an unheld section and read as a flicker ("too fast, slow it down"); these are roughly double, on a screen that is not moving | About §08 · user direction 12 September 2026 · supersedes `partnersDots`' per-rule viewport triggers | — | `thePartners` |
