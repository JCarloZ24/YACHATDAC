 ...
 *
 * ⚑ TWO TONES — 9 September 2026, user direction (the white desktop navbar).
 * `onDark` is the supplied asset unchanged: cream blob, midnight label, for
 * the transparent band over a hero. `onLight` inverts it — black blob, cream
 * label — which is how the mark is drawn in the white bar. The label is
 * therefore painted TWICE: the tone's own colour underneath, and a midnight
 * copy clipped to the water above it, so that wherever the gold has risen the
 * lettering is midnight in both tones. Without that second copy the `onLight`
 * label would be cream on gold (≈1.9:1) for the whole hover.
 */

/** viewBox geometry of the supplied asset. */
const VB_W = 116;
const VB_H = 44;
/** Displacement amplitude of the waterline filter, in viewBox units. Declared
 *  here because the cover radius has to clear it — see below. */
const WATER_SCALE = 14;

/** Covers the whole blob from any entry point.
 *
 *  ⚠ THE DIAGONAL IS NOT ENOUGH. It is ≈124, and this was 132 — eight units of
 *  margin against a filter that displaces the edge by up to `WATER_SCALE` in
 *  either direction. At full radius the roughened edge therefore fell back
 *  INSIDE the blob in places and the base showed through as a rim: invisible
 *  while the base was cream, a black outline once the white navbar's tone
 *  turned it black (reported 10 September 2026). The margin is now the full
 *  displacement plus a little, so the edge is always well outside the shape
 *  and the shimmer can keep moving it without ever showing ground. */
const COVER_R = 124 + WATER_SCALE * 2 + 12;
...
/** Blob and label colours per ground. Midnight is the label everywhere the
 *  water has risen, so it is not listed here — see the clipped copy below. */
const TONES = {
  onDark: { blob: "#F6F6EC", label: "#122449" },
  // #FFFFFF, not canvas cream — the frame's own CONNECT fill (9 Sep 2026).
  onLight: { blob: "#000000", label: "#FFFFFF" },
} as const;
...
export function ConnectButton({
  ...
  className?: string;
  /** Which ground the blob is sitting on. Defaults to the supplied asset. */
  tone?: keyof typeof TONES;
}) {
  const uid = useId();
  const svgRef = useRef<SVGSVGElement | null>(null);
  const rippleRef = useRef<SVGCircleElement | null>(null);
  const rippleClipRef = useRef<SVGCircleElement | null>(null);
  ...
  const waterClipId = `${uid}-water-clip`;
  ...
  /** Both circles — the visible water and the label's clip — move together. */
  const circles = () =>
    [rippleRef.current, rippleClipRef.current].filter(
      (node): node is SVGCircleElement => node !== null,
    );

  const fillFrom = (x: number, y: number) => {
    const ripple = circles();
    ...
    // Duration scales with the radius so the waterline still crosses the blob
    // at the speed it always did — the extra radius is all off-shape.
    gsap.to(ripple, {
      attr: { r: COVER_R },
      duration: 0.65 * (COVER_R / 132),
      ease: "power2.out",
    });
  };
    ...
    >
      ...
      >
        <defs>
          ...
          {/* The label's midnight copy is clipped to the same rising circle.
              A clipPath cannot take a filter, so this edge is the clean one —
              the turbulence shows on the gold itself, which is where it
              reads. */}
          <clipPath id={waterClipId}>
            <circle ref={rippleClipRef} r="0" />
          </clipPath>
        ...
        {/* THE BLOB AND ITS WATER ARE ONE CLIPPED GROUP, and that is the
            whole point of the rect. Drawing the base as its own `<path>` and
            the water as a separate clipped layer gave the shape TWO
            independently antialiased edges: at the boundary the base's own
            half-covered pixels stayed visible under the water's, which read as
            a dark outline once the white navbar's tone made the base black
            (reported 10 September 2026). Inside one clip there is a single
            edge — the clip's — and whatever is topmost at the edge is what
            blends with the page. The rect overhangs the viewBox because the
            supplied path does too, by a fraction of a unit on three sides; it
            is clipped to the blob regardless, so the extra costs nothing. */}
        ...
        {/* CONNECT, baked into the asset: the tone's own colour at rest… */}
        <path d={TEXT_D} fill={paint.label} />
        {/* …and midnight wherever the water has reached, which is the only
            colour that reads on gold. `rippleRef` drives both circles, so the
