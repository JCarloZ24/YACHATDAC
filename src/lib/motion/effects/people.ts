...
export type PeopleStop = { y: number; hold: number; key: string };
...
export function registerPeople(): void {
  ...
  gsap.registerEffect({
    ...
    effect: (_targets: object, config: Record<string, unknown>) => {
      ...
      for (const stop of stops) {
        const duration = Math.max(0, stop.y - from) / viewport;
  ...
  // Y1. Both the photograph and its title use one decoded texture. Only the
  // window into it changes; there is no zoom on the people in the photograph.
  gsap.registerEffect({
    name: "peopleKnockout",
    extendTimeline: true,
    defaults: { duration: 0.65 },
    effect: (_targets: object, config: Record<string, unknown>) =>
      gsap.fromTo(config.state as PeopleState, { knockout: 0 }, {
        knockout: 1, duration: config.duration as number, ease: "none",
      }),
  });
