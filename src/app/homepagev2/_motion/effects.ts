 ...
 *
 * A copy of src/lib/motion/effects/home.ts, plus the homeLoader effect from effects/loading.ts, owned by /homepagev2 alone. The two
 ...
 *
 * These names are deliberately NOT added to `EffectName` in
 * src/lib/motion/effects/index.ts, and this file is deliberately not in that
 * folder. The registry is the site's shared vocabulary; these are one
 * sandbox route's private copies, and listing them there would put the fork
 * back into a file the live homepage depends on. They still cite
 * docs/motion/motion-grammar.md, row for row, because they are the same
 * effects -- if this page's motion diverges, the rows it earns are added
 * there as variants and this note is what says why they are not here.
 */
import gsap from "gsap";
import { homeTruthScenes } from "../_content/home-truth-scenes";
 ...
 */
export function registerHomeV2(): void {
  gsap.registerEffect({
    name: "homeV2Loader",
    defaults: {},
  ...
  gsap.registerEffect({
    ...
    effect: (targets: HTMLElement[], config: { state: { progress: number; portal: number; wonder: number; truth: number; truthSky: number; truthLight: number; belonging: number; landscapeLift: number; landscapeZoom: number }; render: () => void }) => {
      ...
      // ⚠ The one class-based selector in this file. The fork prefixes every
      // class with v2-, so this had to move with the markup; a data hook
      // would not have. If more selectors like it appear, prefer a data
      // attribute -- it survives a restyle, and it survives a fork.
      timeline.to(root.querySelectorAll(".v2-home-painting-copy"),
