...
/**
 * The Living Work loading screen — hard loads and refreshes only.
 *
...
export function LivingWorkLoader() {
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      const gate =
        t > HARD_CAP_MS ? 1 : 0.35 + (fontsReady ? 0.35 : 0) + (windowLoaded ? 0.3 : 0);
  ...
  return (
    ...
    >
      <noscript>
        <style>{`[data-lw-loader]{display:none}`}</style>
      </noscript>
      ...
      <div className="relative w-full px-6 text-center">
        ...
        >
          ...
          <p aria-live="polite" className="mt-8 text-sm tabular-nums text-canvas/60">
            {percent}%
          </p>
        </div>
